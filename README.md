...
The build is a single self-contained `dist/index.html` (art is imported from `src/assets` and inlined, never served from `public`), so it can be opened or hosted anywhere.
